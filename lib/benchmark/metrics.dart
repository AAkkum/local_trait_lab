...
class BenchmarkMetrics {
  ...
  Map<String, Object?> toJson() => <String, Object?>{
        ...
        'per_class': perClass.map((String key, ClassMetrics value) => MapEntry<String, Object?>(key, value.toJson())),
      };
}

BenchmarkMetrics computeBenchmarkMetrics(List<BenchmarkPredictionRow> rows) {
  final List<BenchmarkPredictionRow> successful = rows.where((BenchmarkPredictionRow row) => row.succeeded && row.predictedLabel != null).toList();
  final List<int> latencies = successful.map((BenchmarkPredictionRow row) => row.latencyMs).toList()..sort();
  ...
  final Map<String, Map<String, int>> confusion = <String, Map<String, int>>{
    for (final String actual in kEmotionLabels)
      actual: <String, int>{for (final String predicted in kEmotionLabels) predicted: 0}
  };

  int correct = 0;
  for (final BenchmarkPredictionRow row in successful) {
    confusion[row.groundTruth]![row.predictedLabel!] = confusion[row.groundTruth]![row.predictedLabel!]! + 1;
  ...
  for (final String label in kEmotionLabels) {
    ...
    final int fp = kEmotionLabels.where((String other) => other != label).fold<int>(
          0,
          (int sum, String other) => sum + confusion[other]![label]!,
        );
    final int fn = kEmotionLabels.where((String other) => other != label).fold<int>(
          0,
  ...
  final double accuracy = successful.isEmpty ? 0.0 : correct / successful.length;
  final double averageLatency = latencies.isEmpty ? 0.0 : latencies.reduce((int a, int b) => a + b) / latencies.length;
