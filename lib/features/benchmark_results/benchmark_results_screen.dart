    ...
    return Scaffold(
      ...
      body: run == null
          ...
          : ListView(
              ...
              children: <Widget>[
                Text('Dataset: ${run.dataset.name}', style: Theme.of(context).textTheme.titleLarge),
                Text('Accuracy: ${run.metrics.accuracy.toStringAsFixed(4)}'),
                Text('Macro-F1: ${run.metrics.macroF1.toStringAsFixed(4)}'),
                Text('Average latency: ${run.metrics.averageLatencyMs.toStringAsFixed(2)} ms'),
                Text('Median latency: ${run.metrics.medianLatencyMs.toStringAsFixed(2)} ms'),
                ...
                SingleChildScrollView(
                  ...
                  child: DataTable(
                    columns: <DataColumn>[
                      ...
                      ...run.metrics.confusionMatrix.keys.map((String label) => DataColumn(label: Text(label))),
                    ],
                    rows: run.metrics.confusionMatrix.entries.map((entry) {
                      return DataRow(
                        cells: <DataCell>[
                          DataCell(Text(entry.key)),
                          ...entry.value.values.map((int value) => DataCell(Text(value.toString()))),
                        ],
                      );
                    }).toList(),
                  ),
                ...
                Text('JSON export: ${run.exports.jsonPath ?? 'in-memory only'}'),
              ],
            ),
    );
  }
